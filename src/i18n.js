	...
	read(language, namespace) {
		...
		try {
			...
			if (process.env.NODE_ENV === "development") {
				console.log(
					`[i18next] Using cached translations for '${language}/${namespace}', version: ${version}`
				);
			}
		...
		interpolation: {
			...
			format: (value, format, lng) => {
				if (format === "DATE_FULL") {
					...
					const dateFormat = lng === "ru" ? "d MMMM yyyy 'г.'" : "MMMM d, yyyy";
...
if (process.env.NODE_ENV === "development") {
	...
	i18n.on("missingKey", (lngs, namespace, key) => {
		console.warn(
			`[i18next] Missing key: '${key}' in namespace: '${namespace}' for languages: ${lngs.join(", ")}`
		);
	});
}
